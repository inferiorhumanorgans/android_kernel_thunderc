...
static int msmsdcc_get_status(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned int status;
/* LGE_CHANGE_S [hoseok.kim@lge.com] 2010-09-28 */
	unsigned long flags;
/* LGE_CHANGE_E [hoseok.kim@lge.com] 2010-09-28 */

/* LGE_CHANGE_S [hoseok.kim@lge.com] 2010-09-28 */
	spin_lock_irqsave(&host->lock, flags);	// spin_lock_irq(&host->lock);
/* LGE_CHANGE_E [hoseok.kim@lge.com] 2010-09-28 */
	...
	if (host->plat->status) {
		...
		if (status ^ host->oldstat) {
			...
			host->oldstat = status;
/* LGE_CHANGE_S [hoseok.kim@lge.com] 2010-09-28 */
			spin_unlock_irqrestore(&host->lock, flags);	// spin_unlock_irq(&host->lock);
/* LGE_CHANGE_E [hoseok.kim@lge.com] 2010-09-28 */
