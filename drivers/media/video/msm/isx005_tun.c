...
static int isx005_reg_preview(void)
{
	...
	for(i = 0; i < 300; i++)
	{
		...
		unsigned char pict_out_info_now = 0;

		rc = isx005_i2c_read(isx005_client->addr, 0x0004, &pict_out_info_now, BYTE_LEN);  // Output image mode of current frame 
		rc = isx005_i2c_read(isx005_client->addr, 0x00F8, &cm_changed_status, BYTE_LEN);

		if((cm_changed_status & 0x0002)&&((pict_out_info_now&0x03)==0x00))
		{
			rc = isx005_i2c_write(isx005_client->addr, 0x00FC, 0x1F, BYTE_LEN);  // interrupt clear
...
static int isx005_reg_snapshot(void)
{
	...
	for(i = 0; i < 300; i++)
	{
		...
		if((cm_changed_status & 0x0002)&&((pict_out_info_now&0x03)==0x02))
		{
			rc = isx005_i2c_write(isx005_client->addr, 0x00FC, 0x1F, BYTE_LEN); // interrupt clear
...
static int isx005_cancel_focus(int mode)
{
	...
	/* AF Tuning 할때 Search 범위가 변경 가능성이 있기에 범위를 읽어서 적용하도록 수정 Start */

	unsigned short  AF_SEARCH_AREA_LOW = 0;
	unsigned short  AF_SEARCH_AREA_HIGH = 0;
	
	switch(mode){
	case 0:
		rc = isx005_i2c_read(isx005_client->addr, 0x4800, &AF_SEARCH_AREA_LOW, WORD_LEN);
		lense_po_back = (int)AF_SEARCH_AREA_LOW;
		break;
	
	case 1:
		rc = isx005_i2c_read(isx005_client->addr, 0x4802, &AF_SEARCH_AREA_HIGH, WORD_LEN);
		lense_po_back = (int)AF_SEARCH_AREA_HIGH;
		break;
	}
	/* End */
...
#ifdef SONY_AF_FIX
	rc = isx005_i2c_read(isx005_client->addr, 0x6D76, &af_status, BYTE_LEN);
	if (rc < 0) {
		printk("[isx005.c]%s: fail in reading af_state\n",__func__);
		return rc;
	}
#endif
#endif
...
#ifdef SONY_AF_FIX
	if ((af_result == 1) && (af_status == 0x08)) {
#else
	if (af_result == 1) {
#endif
		*lock = CFG_AF_LOCKED;		// success
		CDBG("isx005_check_focus - CFG_AF_LOCKED, af_result : %d, af_status : %d\n", af_result, af_status);	// SONY_AF_FIX
		return rc;
	} else {
		*lock = CFG_AF_UNLOCKED;	// fail
		CDBG("isx005_check_focus - CFG_AF_UNLOCKED, af_result : %d, af_status : %d\n", af_result, af_status);	// SONY_AF_FIX
		return rc;
	}
...
static int isx005_set_af_start(int mode)
{
	int rc = 0;
	int i =0;
	unsigned char af_lock = 0;
	unsigned char af_status =0;

	CDBG("isx005_set_af_start - prev : %d, mode : %d\n", prev_af_mode, mode);	// SONY_AF_FIX
...
static int isx005_move_focus(int32_t steps)
{
	...
	for(i = 0; i < 24; ++i) {
		rc = isx005_i2c_write(isx005_client->addr, 0x00FC, 0x1F, BYTE_LEN);
		if (rc < 0)
			return rc;
