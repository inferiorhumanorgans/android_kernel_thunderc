...
#if defined (CONFIG_LGE_UNIFIED_LED)
//20100721 yongman.kwon@lge.com MS690 : Fix current level for key backlight [START]
//	ret = leds_pdata->msm_keypad_led_set(value / MAX_KEYPAD_BL_LEVEL);
	if(value == 255)
		ret = leds_pdata->msm_keypad_led_set(0x03);
	else
		ret = leds_pdata->msm_keypad_led_set(value / MAX_KEYPAD_BL_LEVEL);
//20100721 yongman.kwon@lge.com MS690 : Fix current level for key backlight[END]
#else	/* origin */
