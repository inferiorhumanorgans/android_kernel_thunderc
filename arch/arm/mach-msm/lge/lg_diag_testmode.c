...
//20100724 yongman.kwon@lge.com MS690 : not supported Acoustic [START]
//	if(pReq->acoustic >= ACOUSTIC_LOOPBACK_OFF)
//		    pRsp->ret_stat_code = TEST_NOT_SUPPORTED_S;
//20100724 yongman.kwon@lge.com MS690 : not supported Acoustic [END]
...
void* LGF_ExternalSocketMemory(	test_mode_req_type* pReq ,DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	...
	case EXTERNAL_SOCKET_ERASE:
		...
		pRsp->test_mode_rsp.socket_memory_usedsize = 
			((long long)(sf.f_blocks - (long long)sf.f_bfree) * sf.f_bsize);
			break;
...
//20101006 yongman.kwon@lge.com [US670] For charger mode [START]
...
//20101006 yongman.kwon@lge.com [US670] For charger mode [END]
void * LGF_TestModeFboot (	test_mode_req_type* pReq ,DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	printk(KERN_ERR "khlee debug %d \n", pReq->fboot);

/*LGE_CHANGES yongman.kwon@lge.com 2010-09-06 [MS690] : For first boot check in testmode from LS670[START]*/
	pRsp->ret_stat_code = TEST_FAIL_S;
	...
	case FIRST_BOOTING_COMPLETE_CHECK:
		...
		break;
/*LGE_CHANGES yongman.kwon@lge.com 2010-09-06 [MS690] : For first boot check in testmode from LS670 [END]*/
//20101006 yongman.kwon@lge.com [US670] For charger mode [START]
	case FIRST_BOOTING_CHG_MODE_CHECK:
		printk("[INFORPC] : %d\n", first_booting_chg_mode_status);
		
		if(first_booting_chg_mode_status == 1)
			pRsp->ret_stat_code = TEST_OK_S; //charger mode
		else
			pRsp->ret_stat_code = TEST_FAIL_S;//normal mode
		break;
//20101006 yongman.kwon@lge.com [US670] For charger mode [END]
...
// LGE_CHANGE_S [hoseok.kim@lge.com ] 20101004,  It is blocked after setting Factory Reset Flag to 6.
#if 0 
...
#endif 
// LGE_CHANGE_E [hoseok.kim@lge.com ] 20101004,  It is blocked after setting Factory Reset Flag to 6.
...
//20100920 yongman.kwon@lge.com [US670] : this function move to ARM9 for flight mode.
